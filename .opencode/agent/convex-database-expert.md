...
mode: all
model: local/antigravity/claude-opus-4-5
---
...
- When the user reports an error:
  ...
  - Remind users to regenerate `_generated` files via `bunx convex dev` if type references fall behind (or `bunx convex codegen` if dev is already running).
